...
from federatedscope.contrib.workers.laplacian_with_domain_separation_1_out_noGlobal_with_VAE_client import \
    LaplacianDomainSeparationVAE_1_out_onlyDiff_noGlobal_with_VAE_Client
...
def train(lr, kld_ne_imp, diff_imp_global, diff_imp_local, beta, recon_imp, csd_imp):
    ...
    init_cfg.data.save_dir = \
        'Graph-DC_1_out_only_COSINE_Diff_with_VAE_multistep_lr_' + str(lr).replace(
            '.', '_') + '_A'+ str(kld_ne_imp).replace('.', '_') + \
    '_F' + str(diff_imp_global).replace('.', '_') + '_F' + str(diff_imp_local).replace(
        '.', '_') + '_H' + str(csd_imp).replace(
        '.', '_') + '_I' + str(beta).replace(
        '.', '_') + '_J' + str(recon_imp).replace(
        '.', '_')
    """
        kld_ne_imps = [1] #A
        kld_local_imp = 1 #B
            ...
            for beta in betas:
                for recon_imp in recon_imps:
                    for kld_ne_imp in kld_ne_imps:
                        if diff_imp == 0.005 and beta in [0.1, 1] and recon_imp == 1:
                            continue
