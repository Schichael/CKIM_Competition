...
from federatedscope.contrib.trainer.laplacian_trainer_dom_sep_VAE_2_out_prox_loss_NEW import call_laplacian_trainer
...
from federatedscope.contrib.workers.laplacian_with_domain_separation_VAE_2_out_proxLoss_NEW_client import \
    LaplacianDomainSeparationVAE_2_out_proxLoss_NEW_Client
...
metrics = [
    ('recon_loss', call_recon_loss_metric), ('kld_loss_encoder', call_kld_loss_encoder_metric),
    ('kld_global', call_kld_global_metric), ('kld_interm', call_kld_interm_metric), ('kld_local', call_kld_local_metric),
...
def train(lr, kld_ne_imp, kld_local_imp, kld_interm_imp, kld_global_imp, recon_imp, diff_interm_imp,
              diff_local_imp, sim_global_interm_imp, csd_imp, sim_interm_fixed_imp, prox_loss_imp):
    ...
    init_cfg.data.save_dir = 'Graph-DC_FedVAE_2_out_proxLoss_NEW_lr_' + str(lr).replace('.', '_')

    init_cfg.params = CN()
    init_cfg.params.kld_ne_imp = kld_ne_imp
    init_cfg.params.kld_interm_imp = kld_interm_imp
    init_cfg.params.kld_global_imp = kld_global_imp
    ...
    init_cfg.params.kld_local_imp = kld_local_imp
    init_cfg.params.sim_interm_fixed_imp = sim_interm_fixed_imp
    ...
    runner = FedRunner(data=data,
                   ...
                   client_class = LaplacianDomainSeparationVAE_2_out_proxLoss_NEW_Client,
            ...
            for kld_ne_imp in kld_ne_imps:
                for kld_global_imp in kld_global_imps:
                    train(lr, kld_ne_imp, kld_local_imp, kld_interm_imp, kld_global_imp, recon_imp, diff_interm_imp,
                          diff_local_imp, sim, csd_imp, sim, prox_loss_imp)
