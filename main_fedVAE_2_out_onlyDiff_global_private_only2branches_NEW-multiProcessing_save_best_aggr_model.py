...
def train(lr, kld_ne_imp, diff_imp, csd_imp):
    ...
    init_cfg.data.save_dir = 'SAVED_MODEL_Graph-DC_2_out_only_Diff_global_private_only_2_branches_NEW_sim_loss_lr_' + str(lr).replace('.', '_') + '_A'+ str(kld_ne_imp).replace('.', '_') + \
    '_F' + str(diff_imp).replace('.', '_') + '_H' + str(csd_imp).replace('.', '_')
    """
        kld_ne_imps = [1] #A
        kld_local_imp = 1 #B
        kld_interm_imp = 1 #C
