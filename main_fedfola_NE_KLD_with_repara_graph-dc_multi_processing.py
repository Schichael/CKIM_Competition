...
def train(lr, csd_imp, kld_imp):
    ...
    init_cfg.data.save_dir = 'Graph-DC_FedFOLA_NE_KLD_with_repara_lr_' + str(lr).replace('.',
                                                                            '_') + \
        ...
        for csd_imp in csd_imps:
            for kld_imp in kld_imps:
                for i in range(num_trainings):
                    ...
                    processes.append(
                        pool.apply_async(train, args=(lr, csd_imp, kld_imp)))
