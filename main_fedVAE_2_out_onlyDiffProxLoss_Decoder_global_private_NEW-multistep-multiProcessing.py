...
from federatedscope.contrib.trainer.laplacian_trainer_dom_sep_2_out_only_diff_proxLoss_decoder_NEW import \
...
def train(lr, kld_ne_imp, diff_interm_imp, diff_local_imp, prox_loss_imp, csd_imp, recon_imp, kld_interm_imp):
    ...
    init_cfg.data.save_dir = 'Graph-DC_FedVAE_2_out_only_DiffProx_Decoder_global_private_NEW_multistep_sim_loss_lr_' + str(lr).replace('.', '_') + '_A'+ str(kld_ne_imp).replace('.', '_') + \
    ...
    runner = FedRunner(data=data,
                   ...
                   client_class = LaplacianDomainSeparationVAE_2_out_onlyDiffProxLoss_Decoder_NEW_Client,
...
if __name__ == '__main__':
    ...
    diff_interm_imp = 0.0001 #F    HERE  [0.0001, 0.001]
    diff_local_imp = 0.0001 #G
    csd_imp = 10  #H
    prox_loss_imps = [0.01]  #I    HERE   [0.1, 1]
    recon_imps = [0.1]  # J
    kld_interm_imps = [0.1]  # K
