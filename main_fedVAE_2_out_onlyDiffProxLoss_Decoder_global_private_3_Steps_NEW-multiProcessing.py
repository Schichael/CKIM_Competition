...
from federatedscope.contrib.trainer.laplacian_trainer_dom_sep_2_only_diff_proxLoss_decoder_3_steps_NEW import \
...
from federatedscope.contrib.workers.laplacian_with_domain_separation_2_out_onlyDiffProxLoss_Decoder_3_steps_NEW_client import \
    LaplacianDomainSeparationVAE_2_out_onlyDiffProxLoss_Decoder_3_steps_NEW_Client
from federatedscope.contrib.workers.laplacian_with_domain_separation_2_out_onlyDiffProxLoss_Decoder_NEW_client import \
    LaplacianDomainSeparationVAE_2_out_onlyDiffProxLoss_Decoder_NEW_Client
...
metrics = [
    ...
    ('kld_interm', call_kld_interm_metric)
...
def train(lr, kld_ne_imp, diff_interm_imp, diff_local_imp, prox_loss_imp, csd_imp, recon_imp, kld_interm_imp):



    cfg_file = 'scripts/B-FHTL_exp_scripts/Graph-DC/fedDomSep_VAE_global_private_decoder.yaml'
    ...
    init_cfg.data.save_dir = 'Graph-DC_FedVAE_2_out_only_DiffProx_Decoder_global_private_3_steps_NEW_sim_loss_lr_' + str(lr).replace('.', '_') + '_A'+ str(kld_ne_imp).replace('.', '_') + \
    ...
    '_G' + str(diff_local_imp).replace('.', '_') + '_H' + str(csd_imp).replace('.', '_') + '_I' + str(prox_loss_imp).replace('.', '_') + '_J' + \
                             str(recon_imp).replace('.', '_') + '_K' + str(kld_interm_imp).replace('.', '_')+ 'sim_loss_'+ 'prox_loss'
    """
        kld_ne_imps = [1] #A
        kld_local_imp = 1 #B
        kld_interm_imp = 1 #C
    ...
    init_cfg.params.recon_imp = recon_imp
    init_cfg.params.kld_interm_imp = kld_interm_imp
    ...
    runner = FedRunner(data=data,
                   ...
                   client_class = LaplacianDomainSeparationVAE_2_out_onlyDiffProxLoss_Decoder_3_steps_NEW_Client,
...
if __name__ == '__main__':
    ...
    num_trainings = 1
    kld_ne_imps = [0, 0.1, 1] #A
    diff_imps = [0.0001]   #Now 0.0001
    ...
    for lr in lrs:
        for prox_loss_imp in prox_loss_imps:
            for diff_imp in diff_imps:
                for recon_imp in recon_imps:
                    for kld_interm_imp in kld_interm_imps:
                        for kld_ne_imp in kld_ne_imps:
                            processes.append(pool.apply_async(train, args=(lr, kld_ne_imp, diff_imp, diff_imp, prox_loss_imp, csd_imp, recon_imp, kld_interm_imp)))
