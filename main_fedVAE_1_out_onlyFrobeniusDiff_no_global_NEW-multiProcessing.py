...
def train(lr, kld_ne_imp, diff_interm_imp, diff_local_imp, csd_imp):
    ...
    init_cfg.data.save_dir = \
        'Graph-DC_FedVAE_1_out_only_FrobeniusDiff_multistep_NEW_sim_loss_lr_' + str(
    ...
    runner = FedRunner(data=data,
                   ...
                   client_class = LaplacianDomainSeparationVAE_1_out_onlyDiff_noGlobal_NEW_Client,
...
if __name__ == '__main__':
    ...
    diff_global_imps = [0.01, 0.05, 0.002] #F    HERE  [0.0001, 0.001]
    diff_local_imp = 0 #G
    csd_imp = 10 #H
            ...
            for diff_global_imp in diff_global_imps:
                    for kld_ne_imp in kld_ne_imps:
                        for i in range(num_trainings):
                            time.sleep(10)
                            setup_seed(i)
                            processes.append(pool.apply_async(train, args=(lr, kld_ne_imp, diff_global_imp, diff_local_imp, csd_imp)))
