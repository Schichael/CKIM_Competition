    ...
    def update_best_result(self,
        ...
        if not isinstance(new_results, dict):
            ...
            # round_wise_update_key, update others at the same time
                ...
                cur_result = new_results[round_wise_update_key]
                if update_best_this_round or \
                        ('loss' in round_wise_update_key and 'loss' in
                         round_wise_update_key) or \
                        ('std' in round_wise_update_key and 'std' in round_wise_update_key):
                    # The smaller the better
                    if results_type in [
                        "client_best_individual",
                        "unseen_client_best_individual"
                        ...
                        cur_result = min(cur_result)
                    if update_best_this_round or \
                            round_wise_update_key not in best_result or cur_result < \
                            best_result[round_wise_update_key]:
                        best_result[round_wise_update_key] = cur_result
                        update_best_this_round = True
                elif update_best_this_round or \
                        ('acc' in round_wise_update_key and 'acc' in round_wise_update_key) or \
                        ('imp_ratio' in round_wise_update_key and 'imp_ratio' in round_wise_update_key):
                    # The larger the better
                    if results_type in [
                        "client_best_individual",
                        "unseen_client_best_individual"
