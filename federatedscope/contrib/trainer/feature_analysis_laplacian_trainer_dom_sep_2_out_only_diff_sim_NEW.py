...
class FeatureAnalysisLaplacianDomainSeparationVAE_2Out_OnlyDiffSim_NEW_Trainer(GraphMiniBatchTrainer):
    def __init__(self,
                 model,
                 omega,
                 data,
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        out_global_local, out_local_interm, kld_loss_encoder, diff_local_interm, sim_global_interm, x_local, x_interm, x_global = ctx.model(batch,
        ...
        # interm
        file_name = path + '/interm_' + dataset_name + '_' + str(cur_batch_i) + '.pt'
...
def call_laplacian_trainer(trainer_type):
    if trainer_type == 'laplacian_trainer':
        trainer_builder = FeatureAnalysisLaplacianDomainSeparationVAE_2Out_OnlyDiffSim_NEW_Trainer
