...
class LaplacianFedNovaTrainer(GraphMiniBatchTrainer):
    ...
    def train(self, state:int, target_data_split_name="train", hooks_set=None):
        ...
        ), self.get_omega_para(), self.ctx.eval_metrics, self.ctx.num_rounds
...
def call_laplacian_trainer(trainer_type):
    if trainer_type == 'laplacian_trainer':
        trainer_builder = LaplacianFedNovaTrainer
