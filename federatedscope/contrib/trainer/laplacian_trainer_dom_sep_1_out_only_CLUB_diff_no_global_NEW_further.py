...
class LaplacianDomainSeparation_1Out_OnlyCLUBDiff_noGlobal_Further_NEW_Trainer(
    ...
    def _backward_step_old(self, ctx):
        ...
        # train CLUB network
        loss = (self.config.params.club_lr / self.config.train.optimizer.lr) * ctx.diff_local_interm
...
def call_laplacian_trainer(trainer_type):
    if trainer_type == 'laplacian_trainer':
        trainer_builder = LaplacianDomainSeparation_1Out_OnlyCLUBDiff_noGlobal_Further_NEW_Trainer
