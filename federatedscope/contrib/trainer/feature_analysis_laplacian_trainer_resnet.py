...
class FeatureAnalysisLaplacianResNet_Trainer(GraphMiniBatchTrainer):
    def __init__(self,
                 model,
                 omega,
                 data,
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        ctx.diff_loss_2 = diff_loss_1
        ctx.diff_loss_2_metric.append(diff_loss_2.detach().item())
        ...
        file_name = path + '/x_local_1_' + dataset_name + '_' + str(cur_batch_i) + '.pt'
        torch.save(x_local_1, file_name)


        # interm
        file_name = path + '/x_local_2_' + dataset_name + '_' + str(cur_batch_i) + '.pt'
        torch.save(x_local_2, file_name)

        file_name = path + '/x_global_1_' + dataset_name + '_' + str(cur_batch_i) + '.pt'
...
def call_laplacian_trainer(trainer_type):
    if trainer_type == 'laplacian_trainer':
        trainer_builder = FeatureAnalysisLaplacianResNet_Trainer
