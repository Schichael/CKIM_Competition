...
class LaplacianDomainSeparationReconTrainer(GraphMiniBatchTrainer):
    def __init__(self,
                 model,
                 omega,
                 data,
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        pred, out_enc_shared, out_enc_private, recon_loss = ctx.model(batch)
        diffloss = DiffLoss()
        ...
        ctx.diff_loss = diffloss(out_enc_shared, out_enc_private)
    ...
    def _hook_on_batch_backward(self, ctx):
        ...
        ctx.optimizer.zero_grad()
        #print(f"task loss: {ctx.loss_batch_ce}")
        #print(f"recon loss: {ctx.recon_loss}")
...
def call_laplacian_trainer(trainer_type):
    if trainer_type == 'laplacian_trainer':
        trainer_builder = LaplacianDomainSeparationReconTrainer
