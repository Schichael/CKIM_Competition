...
class LaplacianDomainSeparationVAE_2Out_OnlyDiffMINE_Sim_NEW_Trainer(GraphMiniBatchTrainer):
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        #print(f"diff_local_interm: {ctx.diff_local_interm_metric}")
    ...
    def _backward_step(self, ctx):
        ...
        for param in ctx.model.named_parameters():
            if (param[0].startswith("interm") or param[0].startswith("mine")):
                param[1].requires_grad = False
        ...
        # Do not use prior loss for global branch
        for param in ctx.model.named_parameters():
            if (param[0].startswith("global")):
                param[1].requires_grad = False
    ...
    def _backward_step_MINE(self, ctx):
        """ Only to train the MINE network
    ...
    def _hook_on_batch_backward(self, ctx):
        ...
        ctx.optimizer.zero_grad()
        if self.mineTrainMode:
            self._backward_step_MINE(ctx)
        else:
