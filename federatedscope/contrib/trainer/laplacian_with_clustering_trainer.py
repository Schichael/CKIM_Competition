...
class LaplacianClusteringTrainer(GraphMiniBatchTrainer):
    def __init__(self,
                 model,
                 omega,
                 data,
    ...
    def update(self, content, strict=False):
        ...
        for i, cluster in enumerate(clusters):
            if self.ID in cluster[0]:
                cluster_id = i
                break

        new_model, new_omega = clusters[cluster_id][1:]
...
def call_laplacian_trainer(trainer_type):
    if trainer_type == 'laplacian_trainer':
        trainer_builder = LaplacianClusteringTrainer
