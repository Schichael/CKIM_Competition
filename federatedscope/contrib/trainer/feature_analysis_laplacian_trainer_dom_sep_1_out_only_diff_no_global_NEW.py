...
class FeatureAnalysisLaplacianDomainSeparationVAE_1Out_OnlyDiff_noGlobal_NEW_Trainer(GraphMiniBatchTrainer):
    def __init__(self,
                 model,
                 omega,
                 data,
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        out_global_local, out_interm, kld_loss_encoder, diff_local_interm, x_local, x_global = ctx.model(batch)
...
def call_laplacian_trainer(trainer_type):
    if trainer_type == 'laplacian_trainer':
        trainer_builder = FeatureAnalysisLaplacianDomainSeparationVAE_1Out_OnlyDiff_noGlobal_NEW_Trainer
