...
class \
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        out_local_out, out_local_interm, out_interm, kld_loss_encoder, \
        ...
        ctx.loss_batch = ctx.criterion(out_local_out, label)
        ctx.loss_out_local_out = ctx.criterion(out_local_out, label)
    ...
    def _backward_step(self, ctx):
        ...
        loss = ctx.loss_out_local_out + self.config.params.kld_ne_imp * \
               ...
               self.config.params.sim_imp * ctx.prox_loss
        loss.backward(retain_graph=True)
...
def call_laplacian_trainer(trainer_type):
    if trainer_type == 'laplacian_trainer':
        trainer_builder = LaplacianDomainSeparationVAE_1Out_OnlyDiff_ProxSim_global_private_NEW_Trainer
