...
class LaplacianDomainSeparationVAE_2Out_proxLoss_NEW_Trainer(GraphMiniBatchTrainer):
    def __init__(self,
                 model,
                 omega,
                 data,
    ...
    def _backward_step_3(self, ctx):
        ...
        loss = self.config.params.prox_loss_imp * ctx.prox_loss
        loss.backward(retain_graph=False)
...
def call_laplacian_trainer(trainer_type):
    if trainer_type == 'laplacian_trainer':
        trainer_builder = LaplacianDomainSeparationVAE_2Out_proxLoss_NEW_Trainer
