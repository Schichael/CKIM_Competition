...
class LaplacianTrainer_SWD_MMD(GraphMiniBatchTrainer):
    def __init__(self,
                 model,
                 omega,
                 data,
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        pred, swd_loss, mmd_loss = ctx.model(batch)
        ...
        ctx.swd_loss = swd_loss
        ctx.swd_loss_metric = swd_loss.detach().item()

        ctx.mmd_loss = mmd_loss
        ctx.mmd_loss_metric = mmd_loss.detach().item()
...
def call_laplacian_trainer(trainer_type):
    if trainer_type == 'laplacian_trainer':
        trainer_builder = LaplacianTrainer_SWD_MMD
