...
class \
    ...
    def _align_global_local_parameters(self, model):
        for name, param in deepcopy(model).named_parameters():
            if name.startswith('local'):
                ...
                global_name = 'llocal_out' + stripped_name
    ...
    def update(self, content, strict=False):
        ...
        if self.first_round:
            ...
            self._align_global_interm_parameters(self.ctx.model)
    ...
    def _hook_on_fit_start_init(self, ctx):
        ...
        ctx.num_local_out_features_not_0_metric = []
        ctx.avg_local_out_features_not_0_metric = []
        ctx.num_features_global_local_out_metric = []
        ctx.num_features_local_out_local_metric = []
        ctx.sim_interm_local_out_metric = []
        ...
        if self.round_num < 2:
            ...
            self._align_global_interm_parameters(ctx.model)
    ...
    def stats_calculator(self, local_features, local_out_features, global_features):
        local_features = local_features.cpu().detach().numpy()
        local_out_features = local_out_features.cpu().detach().numpy()
        ...
        mult_local_out_global = local_out_features * global_features
        mult_local_out_local = local_out_features * local_features
        ...
        num_local_out_features_not_0 = np.sum(local_out_features != 0) / \
                                    num_total_features_curr
        avg_local_out_features_not_0 = local_out_features.sum() / np.sum(local_features
                                                                     != 0)
        ...
        num_features_global_local_out = np.sum(
            mult_local_out_global != 0) / num_total_features_curr
        num_features_local_out_local = np.sum(
            mult_local_out_local != 0) / num_total_features_curr
        ...
        return num_local_features_not_0, avg_local_features_not_0, \
            ...
            num_local_out_features_not_0, avg_local_out_features_not_0, \
            num_features_global_local, num_features_global_local_out, num_features_local_out_local

    def _hook_on_batch_forward(self, ctx):
        self.tmp += 1
        batch = ctx.data_batch.to(ctx.device)
        out_local_out_local, out_local_interm, kld_loss_encoder, \
        ...
        ctx.diff_local_global_metric.append(diff_local_interm.detach().item())

        ctx.diff_local_local_out_metric.append(diff_local_local_out.detach().item())

        ctx.sim_interm_local_out = sim_interm_local_out
        ctx.sim_interm_local_out_metric.append(sim_interm_local_out.detach().item())
        ...
        ctx.loss_batch = ctx.criterion(out_local_out_local, label)
        ctx.loss_out_local_local_out = ctx.criterion(out_local_out_local, label)
        ...
        ctx.y_true = label
        ctx.y_prob = out_local_out_local
        ...
        num_local_features_not_0, avg_local_features_not_0, \
            ...
            num_local_out_features_not_0, avg_local_out_features_not_0, \
            num_features_global_local, num_features_global_local_out, num_features_local_out_local = \
            self.stats_calculator(x_local, x_local_out, x_interm)
        ...
        if self.round_num == 0 or self.round_num == 1 or self.round_num == 498 or \
            ...
            # local_out
            file_name = path + '/llocal_out_' + dataset_name + '_' + str(cur_batch_i)\
                        + \
                        '.pt'
            torch.save(x_local_out, file_name)

            # interm
            file_name = path + '/global_' + dataset_name + '_' + str(cur_batch_i) + '.pt'
            torch.save(x_interm, file_name)
    ...
    def _backward_step_orig(self, ctx):
        ...
        loss = ctx.loss_out_local_local_out + self.config.params.kld_ne_imp * ctx.kld_loss_encoder
    ...
    def _backward_step(self, ctx):
        ...
        # loss for output and KLD
        loss = ctx.loss_out_local_local_out + self.config.params.kld_ne_imp * \
...
def call_laplacian_trainer(trainer_type):
    if trainer_type == 'laplacian_trainer':
        trainer_builder = LaplacianDomainSeparationVAE_2Out_Only_COSINE_Diff_MSE_Sim_global_private_NEW_Trainer
