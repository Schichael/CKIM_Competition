...
from federatedscope.gfl.gcflplus.utils import norm

logger = logging.getLogger(__name__)


class GCFL_PLUS_LaplacianDomainSeparationVAE_2_out_onlyDiffSim_NEW_Client(Client):
    def __init__(self,
                 ID=-1,
                 server_id=None,
                 state=-1,
        ...
        trainer = LaplacianDomainSeparationVAE_2Out_OnlyDiffSim_NEW_Trainer(
            model=model,
    ...
    def callback_funcs_for_model_para(self, message: Message):
        ...
        dW = dict()
        for key in model_para_all.keys():
            dW[key] = model_para_all[key] - W_old[key].cpu()
        ...
        self.comm_manager.send(
            Message(msg_type='model_para',
                    ...
                    receiver=[sender],
                    state=self.state,
                    content=(sample_size, model_para_all, omega_set, dW, convGradsNorm)))
