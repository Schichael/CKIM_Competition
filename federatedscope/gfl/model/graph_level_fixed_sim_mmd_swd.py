...
from federatedscope.gfl.model.mmd_loss import MMD_loss
...
class GNN_Net_Graph(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # Output layer
        self.global_linear = Sequential(Linear(hidden, hidden), torch.nn.ReLU())
        self.fixed_linear = Sequential(Linear(hidden, hidden), torch.nn.ReLU())
        self.clf = Linear(hidden, out_channels)

        for param in self.fixed_gnn.named_parameters():
            param[1].requires_grad = False

        for param in self.fixed_linear.named_parameters():
            param[1].requires_grad = False
    ...
    def forward(self, data):
        ...
        x_fixed = self.fixed_gnn((x, edge_index))
        x_global = self.pooling(x_global, batch)
        x_fixed = self.pooling(x_fixed, batch)
        x_global = self.global_linear(x_global)
        x_fixed = self.fixed_linear(x_fixed)
        swd_loss = swd(x_global, x_fixed)
        mmd_loss = self.mmd_loss(x_global, x_fixed)

        x_global = F.dropout(x_global, self.dropout, training=self.training)
        x_global = self.clf(x_global)
        return x_global, swd_loss, mmd_loss
