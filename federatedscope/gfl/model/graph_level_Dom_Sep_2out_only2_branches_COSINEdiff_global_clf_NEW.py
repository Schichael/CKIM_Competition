...
# graph_level_Dom_Sep_2out_only2_branches_COSINEdiff_global_clf_NEW
...
class GNN_Net_Graph(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # local
        self.clf = Linear(hidden, out_channels)
        self.global_classifier = Linear(hidden, out_channels)
    ...
    def forward(self, data):
        ...
        x_local_out = F.dropout(x_local, self.dropout, training=self.training)
        x_global_out = F.dropout(x_global, self.dropout, training=self.training)

        x_local_global = x_local_out + x_global_out
        out_local_global = self.clf(x_local_global)
        out_global = self.global_classifier(x_global_out)
        ...
        return out_local_global, out_global, kld_loss_encoder, diff_local, \
            diff_global, x_local, x_global
