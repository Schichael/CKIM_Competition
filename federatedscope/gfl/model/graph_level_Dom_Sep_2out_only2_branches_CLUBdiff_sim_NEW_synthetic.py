...
# graph_level_Dom_Sep_2out_only2_branches_CLUBdiff_sim_NEW_synthetic
...
class GNN_Net_Graph(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        x_s = np.random.multivariate_normal(mean=[0, 0],
                                          cov=[[1, 0.8], [1, 0.8]],
                                          size=(64, 64))

        x_1 = x_s[:,:,0]
        x_2 = x_s[:,:,1]
        x_1 = torch.Tensor(x_1).to('cuda:0')
        x_2 = torch.Tensor(x_2).to('cuda:0')
        ...
        diff_local_global = self.club_diff.learning_loss(x_1, x_2)

        with torch.no_grad():
            MI = self.club_diff(x_1, x_2)
        ...
        return out_local_global, kld_loss_encoder, diff_local_global, MI
